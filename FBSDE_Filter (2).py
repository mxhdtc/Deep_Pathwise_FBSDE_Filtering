...
def build_Zakai_Solver(config):
    (encoder_config, decoder_config), is_score = config
    ...
    # else:
    #     decoder_model_layers.append(
    #         tf.keras.layers.Dense(units=decoder_output_dim, activation='exponential',
    #                               kernel_initializer=tf.keras.initializers.LecunNormal(seed=np.random.randint(1e9)))
    #     )
...
class Deep_FBSDE_Filter(object):
    def __init__(self, delta_t, T, drift, diffusion, obv_drift, sig_dim, obv_dim, init_dis, nn_config, build_neural_networks,
        ...
        if self.is_score:
            print('Using score function method!')
        else:
            print('Using PINN method!')
        def build_models():
            if from_saved_models:
                ...
                self.Value_function_solver, self.Obv_encoder = self.build_neural_networks((self.nn_config, self.is_score))
    ...
    def dist_train_controlled_sde(self, config):
        ...
        with ((self.strategy.scope())):
            ...
            def robust_running_loss(trajectories, observation_trajectories, brownian_motion):
                return 
            def fbsde_trajectory_running_loss(trajectories, obv_trajectoies, brownian_motion):
                ...
                def accumulate_internal_cost(accumulate_internal, inputs):
                    ...
                    if self.is_score:
                        nabla_log_v = self.Value_function_solver(tf.concat([x, t_batch, y_hidden_t], axis=-1))
                    else:
                        with tf.GradientTape() as tape:
                            tape.watch(x)
                            log_v = self.Value_function_solver(tf.concat([x, t_batch, y_hidden_t], axis=-1))
                            nabla_log_v = tape.gradient(log_v, x)
                    martingale_increment = tf.sqrt(self.delta_t) * \
                                           tf.einsum('ij,ij->i', tf.einsum('ijk, ik->ij', sigma_T,
                                           nabla_log_v), dw)
                    # martingale_increment = tf.sqrt(self.delta_t) * \
                    #                        tf.einsum('ij,ij->i', tf.einsum('ijk, ik->ij', sigma_T,
                    #                        self.Value_function_solver(tf.concat([x, t_batch, y_hidden_t], axis=-1))), dw)
